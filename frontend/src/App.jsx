...
function Contact() {
  const [status, setStatus] = useState(''); const [sending, setSending] = useState(false); const [form, setForm] = useState({ name: '', email: '', message: '', website: '' });
  const submit = async event => {
    event.preventDefault(); setStatus(''); setSending(true);
    try { await apiRequest('/contact', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) }); setStatus('Cảm ơn bạn! Tin nhắn đã được gửi đến Sean.'); setForm({ name: '', email: '', message: '', website: '' }); }
    catch (reason) { setStatus(reason.message || 'Không thể gửi tin nhắn lúc này. Vui lòng thử lại sau.'); }
    finally { setSending(false); }
  };
  return <section className="section contact-page"><div><p className="section-kicker">LIÊN HỆ</p><h1 className="page-title">Hãy tạo nên điều<br />đáng nhớ.</h1><p className="page-lead">Có ý tưởng, dự án hoặc chỉ muốn nói lời chào? Tôi luôn sẵn lòng kết nối.</p><a className="email-link" href="mailto:nguyenhuutoan1906@gmail.com">nguyenhuutoan1906@gmail.com <FaArrowRight /></a><div className="socials"><a href="https://github.com/nguyenhuutoan1906" target="_blank" rel="noreferrer" aria-label="GitHub"><FaGithub /></a><a href="https://facebook.com/nguyenhuutoan1906" target="_blank" rel="noreferrer" aria-label="Facebook"><FaFacebookF /></a><a href="https://instagram.com/sean_nht196" target="_blank" rel="noreferrer" aria-label="Instagram"><FaInstagram /></a></div></div><form className="contact-form" onSubmit={submit}>{status && <div className={status.startsWith('Cảm') ? 'success-message' : 'image-error'}>{status}</div>}<input className="contact-honeypot" tabIndex="-1" autoComplete="off" aria-hidden="true" value={form.website} onChange={e => setForm({ ...form, website: e.target.value })} /><label>Họ và tên<input required maxLength="100" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Tên của bạn" /></label><label>Email<input required type="email" maxLength="254" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="ban@email.com" /></label><label>Nội dung<textarea required maxLength="3000" rows="5" value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} placeholder="Bạn muốn cùng xây dựng điều gì?" /></label><button className="button primary" type="submit" disabled={sending}>{sending ? 'Đang gửi...' : <>Gửi lời nhắn <FaPaperPlane /></>}</button></form></section>;
}
